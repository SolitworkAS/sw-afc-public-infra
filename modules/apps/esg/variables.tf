...
variable "name_database_carbacc" {
  default     = "carbacc-db"
  description = "database name, must only contain lowercase letters, numbers and dashes"
  validation {
    condition     = can(regex("^[a-z0-9-]+$", var.name_database_carbacc))
    error_message = "name_database_carbacc must only contain lowercase letters, numbers and dashes"
  }
}
